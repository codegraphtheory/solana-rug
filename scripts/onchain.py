...
def resolve_deployer(mint: str) -> str | None:
    ...
    fee_payer = tx.get("transaction", {}).get("message", {}).get("accountKeys", [{}])[0].get("pubkey", "")
...
def check_honeypot(mint: str) -> HoneypotResult:
    ...
    try:
        ...
        url = f"https://quote-api.jup.ag/v6/quote?inputMint={mint}&outputMint={wsol}&amount=1000000&slippageBps=500"
...
def estimate_token_age(mint: str) -> tuple[int, list[str]]:
    ...
    if dex_age is not None and dex_age > age_days + 1:
        actual_age = dex_age
        warnings.append(
            f"RPC returned only {age_days:.1f}d of history; actual age from DEX indexer: ~{actual_age:.1f}d"
        )
